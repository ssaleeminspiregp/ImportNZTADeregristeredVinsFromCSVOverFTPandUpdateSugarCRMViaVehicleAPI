...
class StorageWriter:
    ...
    def upload_raw(self, local_path: Path) -> StoredFile:
        ...
        return StoredFile(blob_name=blob_name, uri=f"gs://{self.bucket}/{blob_name}")
    ...
    def _move(self, stored: StoredFile, destination_prefix: str, label: str) -> StoredFile:
        ...
        destination_name = self._build_blob_name(destination_prefix, Path(stored.blob_name).name)
        ...
        return StoredFile(blob_name=destination_name, uri=f"gs://{self.bucket}/{destination_name}")

    def _build_blob_name(self, prefix: str, filename: str) -> str:
        timestamp = datetime.datetime.utcnow().strftime("%Y/%m/%d/%H%M%S")
        cleaned_prefix = prefix.strip("/")
        return f"{cleaned_prefix}/{timestamp}-{filename}"
